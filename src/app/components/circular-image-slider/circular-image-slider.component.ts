...
sustainable future for all of us.`,
  ...
  ngAfterViewInit() {
    ...
    if (this.containerRefR && this.containerRefR.nativeElement) {
      ...
      this.radiusRight =
        Math.min(this.containerWidth, this.containerHeight) * 0.5;
    }
  ...
  setActive(index: number) {
    ...
    const matchIndex = this.imagesRight.findIndex(
  ...
  setDescripription(index: number) {
    const jsonTitle = this.images[index]?.title;
    const mappedId = this.titleMapping[jsonTitle];
    if (mappedId) {
      this.descripTitle.set(this.titlesText()[mappedId].title);
      this.descripText.set(this.titlesText()[mappedId].description);
    }
  }
  ...
  getImagePositionL(index: number): string {
    ...
    const rotation =
      index === this.activeIndex ? " " : ` rotate(${rotationAngle}deg)`;
